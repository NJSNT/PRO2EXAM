...
public class BorrowStrategy implements MessageStrategy {
    @Override
    public NetworkMessage execute(NetworkMessage request, VinylLibrary library, String clientIp) {
        ...
        synchronized (library) {
            ...
            if (vinyl.isBorrowed()) {
                return NetworkMessage.createError("Vinyl '" + title + "' is already borrowed by " + vinyl.getBorrowingUserId() + ".", username);
            }

            if (vinyl.isReserved() && !username.equalsIgnoreCase(vinyl.getReservingUserId())) {
                return NetworkMessage.createError("Vinyl '" + title + "' is reserved by another user (" + vinyl.getReservingUserId() + ").", username);
            }

            library.borrowVinyl(vinyl, username);

            if (username.equalsIgnoreCase(vinyl.getBorrowingUserId())) {
                return null; // Handled by server broadcast
            } else {
                return NetworkMessage.createError("Failed to borrow vinyl '" + title + "'. Check current state.", username);
            }
        }
    }
}
