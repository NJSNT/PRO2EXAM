...
public class UserSimulator implements Runnable {
    ...
    public UserSimulator(VinylLibrary library, String userId) {
    ...
    public void run() {
        while (running) {
            try {
                ...
                // 1. 库存检查
                if (library.getVinyls().isEmpty()) {
                    continue;
                }
                
                // 2. 随机抽取目标实体
                int index = random.nextInt(library.getVinyls().size());
                Vinyl targetVinyl = library.getVinyls().get(index);
